...
var wcsdmTableRates = {
	...
	sortRateRows: function () {
		...
		var $rowEditing = $('#wcsdm-table-dummy tbody tr.editing');

		if ($rowEditing.length) {
			$rowEditing.removeClass('editing').addClass('highlighted');
			var modalHeight = $rowEditing.closest('.wc-modal-shipping-method-settings').outerHeight();
			var formHeight = $rowEditing.closest('form').outerHeight();
			var rowPositionTop = $rowEditing.position().top;
			var formPositionTop = $rowEditing.closest('form').position().top;
			var scrollHeight = rowPositionTop > 0 ? rowPositionTop + (modalHeight / 2) : (formHeight - Math.abs(formPositionTop)) - (modalHeight / 2);
			$('.wc-modal-shipping-method-settings').animate({
				scrollTop: scrollHeight,
			}, 500, function () {
				setTimeout(function () {
					$rowEditing.removeClass('highlighted');
				}, 700);
			});
